...
--8
--Escribir una sentencia SELECT que devuelva el número de orden, fecha de orden, número de cliente y fecha de embarque de todas las órdenes que no han sido pagadas (paid_date es nulo), pero fueron embarcadas (ship_date) durante los primeros seis meses de 2015.
...
--10
--Obtener el precio máximo, mínimo y precio promedio pagado (ship_charge) por todos los embarques. Se pide obtener la información de la tabla ordenes (orders).
SELECT MAX(ship_charge) AS 'Precio máximo pagado',
	   MIN(ship_charge) AS 'Precio mínimo pagado',
...
--13
--Mostrar fecha de embarque (ship_date) y cantidad total de libras (ship_charge) por día, de aquellos días cuyo peso de los embarques superen las 30 libras. Ordenar el resultado por el total de libras en orden descendente.
