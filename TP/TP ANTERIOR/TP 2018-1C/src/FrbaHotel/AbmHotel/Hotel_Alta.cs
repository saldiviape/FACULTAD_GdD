...
namespace FrbaHotel.AbmHotel
{
    public partial class Hotel_Alta : Form
    {
        ...
        {
            if (Convert.ToDateTime(dtp_fechaCreacion.Value) < Properties.Settings.Default.FechaDelSistema)
                try
                {
                    string cmd = string.Format("EXEC DEVOLVESELA_A_MESSI.altaHotel '{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}', '{7}', '{8}'", txt_nombre.Text.Trim(), txt_mail.Text.Trim(), txt_telefono.Text.Trim(), txt_direccion.Text.Trim(), cmb_estrellas.Text.Trim(), cmb_recargaEstrellas.Text.Trim(), txt_ciudad.Text.Trim(), txt_pais.Text.Trim(), Convert.ToDateTime(dtp_fechaCreacion.Value.ToString()));
                ...
                MessageBox.Show("Ingrese una fecha de creación valida");
                return false;
        }
